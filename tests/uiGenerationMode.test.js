...
test('UI labels selected-scope generation as the only active Phase 1 action', () => {
  ...
  assert.match(displaySources, /Output: Selected Scope Mask/);
  assert.match(displaySources, /AppDB collection/);
  assert.match(displaySources, /ccm_selected_scope_mask/);
  assert.match(displaySources, /current Store, Metric, and Period Lens only/);
  assert.match(displaySources, /intended for Phase 1 validation/);
  assert.match(displaySources, /not production full-mask processing/);
  assert.match(uiSource, /helperText\.selectedScopeMask/);
