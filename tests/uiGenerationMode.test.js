...
test('UI labels selected-scope generation as the only active Phase 1 action', () => {
  ...
  assert.match(displaySources, /current Store and Metric scope/);
  assert.match(displaySources, /all six approved period types/);
  assert.match(displaySources, /not production full-mask processing/);
  assert.match(uiSource, /six approved period types are generated by default/);
