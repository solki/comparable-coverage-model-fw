...
export function createApp(root) {
  ...
  function renderGenerateMask() {
    ...
    return `
      <section class="panel step-card">
        <div class="panel-heading">
          <h2>Commit CCM Mask ${renderInfoTooltip(getTooltipCopy('comparableCoverage'))}</h2>
          ${statusBadge(step?.status || 'locked')}
        </div>
        <p class="note">Commits the loaded working data as the CCM mask to AppDB. All six approved period types are included.</p>
  ...
  function markScopeChanged() {
    ...
    state.l4lRows = [];
    state.l4lValidation = { valid: true, missingFields: [] };
    state.l4lMessage = 'Applied scope changed. The saved CCM mask is stale. Commit a new mask before preparing comparison facts.';
    state.status = 'Applied scope changed. Commit an updated CCM mask in Stage 4 before running the Workflow.';
  }
